...
| `paper_slides_render.py` (`preflight`, `parse`, `narrate`, `render`, `verify`) | A (skill-local gate) | `/paper-slides-render` cannot produce a watchable narrated MP4 without these checks; `verify` exit 2 blocks the "render complete" claim on missing audio, wrong codec, missing faststart, or planned-vs-actual duration drift outside the configured tolerance. Canonical location is `skills/paper-slides-render/scripts/paper_slides_render.py` (Arch C, self-contained — no `tools/` shim). |
...
| paper-slides-render bridge | `paper_slides_render.py preflight --workspace <cwd>` returns `ok=true` AND `slides/TALK_SCRIPT.md` exists | `paper_slides_render.py` (`preflight`, `parse`, `narrate`, `render`, `verify`) | `slides/render/presentation.mp4`, `render.json`, `verify.json` (+ optional `subtitles.srt`) | Phase 0 / Phase 1 / Phase 3 gates in `paper-slides-render` | `paper_slides_render.py render --slides-pdf <pdf> --talk-script <md> --output <mp4>` | `paper_slides_render.py verify --video <mp4> --talk-script <md>` (skill-local gate; exit 2 blocks render-complete claim on codec / faststart / duration drift) |
